...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
		...
		if(this.my_is_empty() == true) {
			 ...
			 * How this recursion is working is that first it will put head into this and put the two trees on both sides into the
			 * variables. Then it will put the trees below them into the variables until it hits the recursive case and adds 1 all
			 * the way up.
			 * So: c1 = tree with root 5 and below and c2 = tree with root 11 and below and this works until level = 0
			 * then it hits base case and result will be 1 and this will be returned into either c1 or c2
			 */
			int c1 = this.my_left_tree().my_count_at_level(level-1);
			int c2 = this.my_right_tree().my_count_at_level(level-1);
			res = c1 + c2;
		}
	...
	public boolean my_is_balanced(){
		...
		if(this.my_is_empty()) {
			...
			if(0 <= this.my_left_tree().my_length() - this.my_right_tree().my_length() && 
					this.my_left_tree().my_length() - this.my_right_tree().my_length() <= 1) {
				boolean check1 = this.my_left_tree().my_is_balanced();
				boolean check2 = this.my_right_tree().my_is_balanced();
				res = check1 && check2;
			}
			else {
				res = false;
	...
	public int my_count_smaller_nodes(T1 key){
		...
		//TO-DO
		int res = 0;
		// This checks if the key is smaller than the root
		if(this.my_is_empty()){
			return res;
		}
		else if(key.compareTo(this.root.getKey()) == 1) {
			res += this.my_left_tree().my_node_count()+1;
	...
	public int my_find_node_at_level(T1 key){
		...
		// first.compareTo(second) answers: if first is less than second it returns -1, equalto = 0, greater than = 1
		...
		if(this.my_is_empty()) {
			...
			else if(key.compareTo(this.my_right_tree().my_root().getKey()) == 0) {
				res++;
			}
			else {
				res++;
				res += this.my_right_tree().my_find_node_at_level(key);
			}
		}
			...
			else if(key.compareTo(this.my_left_tree().my_root().getKey()) == 0) {
				res++;
			}
			else {
				res++;
				res += this.my_left_tree().my_find_node_at_level(key);
			}
		}
		else {
			// This should be if it equals head
			res = 0;
		}
